%%%%Para la realización de este código, primero se ejecuta limpiando la
...
%%%%Se establece la comunicación del puerto en que se encuentra
%el arduino con matlab. Y que primero se tiene que compilar y cargar el
%código en arduino para que despues podamos compilar otro codigo desde
%matlab con el puerto indicado desde el otro ide.
...
%%%%En esta parte se comienza a declarar los valores a introducir, de cada
%%%%eslabon.
...
%%%% Aqui se describe la estructura cinemática que contiene 3 articulaciones
%%%% con 3 grados de libertad, establecen la ubicación de los sistemas de
%%%% referencia de los eslabones, de este sistema robótico articulado.
d1 = l1;
d2 = 0;
d3 = 0;
...
p1 = [0,0,0];
%%%% Cambiar de bit a voltaje. Esta condición contiene una expreción
%%%% matricial con rotación en el eje z, obteniendo la matriz de Denavit 
%%%% Hartenberg, aplicando los parámetros
%%%% que se otuvieron anteriormente obteniendo nueva matriz  y
%%%% multiplicando estas 3 matrices A1 con A2, el resultado de ambas se
%%%% multiplica con A3.
while 1 
    clf
    printAxis();
